#!/usr/bin/env bash
#SBATCH --job-name=cbramod_eo_optuna
#SBATCH --output=/home/mat/projects/EEG_psychostimulant/logs/cbramod_eo_optuna_%j.out
#SBATCH --error=/home/mat/projects/EEG_psychostimulant/logs/cbramod_eo_optuna_%j.err
#SBATCH --partition=gpubase_bygpu_b2
#SBATCH --gres=gpu:1
#SBATCH --mem=128G
#SBATCH --cpus-per-task=16
#SBATCH --time=12:00:00
#SBATCH --array=0-4%5

# Activate environment
source /home/mat/projects/EEG_psychostimulant/.venv/bin/activate

# Default paths
DATA_ROOT="/home/mat/scratch/preproc/"
LABEL_CSV="/home/mat/scratch/epilepsy_label_cleaned.csv"
SAVE_DIR="/home/mat/projects/EEG_psychostimulant/data/results/finetuned_param/cbramod"
WEIGHTS_PATH="/home/mat/CBraMod/pretrained_weights/pretrained_weights.pth"

# Target Condition
COND="EO_baseline"
FOLD=$SLURM_ARRAY_TASK_ID

echo "Starting CBraMod Optuna Fine-tuning Fold $FOLD"
echo "  - Condition: $COND"
echo "  - Save Directory: $SAVE_DIR"

echo "===================================================="
echo "RUNNING Optuna TUNING FOR EO_BASELINE | FOLD $FOLD"
echo "===================================================="

python3 /home/mat/projects/EEG_psychostimulant/eeg_adhd_epilepsy/dl/cbramod/cbramod_finetune.py \
    --data-root "$DATA_ROOT" \
    --label-csv "$LABEL_CSV" \
    --weights-path "$WEIGHTS_PATH" \
    --save-dir "$SAVE_DIR" \
    --conditions "$COND" \
    --fold "$FOLD" \
    --tune \
    --n-trials 20 \
    --seed 42

echo "Finished processing Fold $FOLD at $(date)"
