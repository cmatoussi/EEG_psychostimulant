#!/usr/bin/env python3
"""
Evaluation script for CBraMod embeddings.
Supports standard and all-layers configurations.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate CBraMod embeddings with classification")
    parser.add_argument("--embeddings_dir", type=str, default="/home/mat/scratch/extracted_embeddings/cbramod", help="Path to CBraMod embeddings root")
    ...
    parser.add_argument("--stage", type=str, default="base", help="Preprocessing stage filter (CBraMod usually uses 'base')")
    parser.add_argument("--all_layers", action="store_true", help="Use all_layers embeddings")
    parser.add_argument("--layer_idx", type=int, default=None, help="Specific layer to evaluate if using all_layers. Starts at 0.")
    ...
    parser.add_argument("--json_dir", type=str, default=None, help="Optional directory to save individual JSON result files")
    return parser.parse_args()

def discover_conditions(model_dir: Path, metadata_df: pd.DataFrame, desc: str, all_layers: bool) -> List[str]:
    """
    Lightning-fast discovery of conditions by targeting specific subject metadata files.
    """
    conditions: Set[str] = set()
    logger.info(f"Rapid scanning for CBraMod conditions...")
    ...
    for sub_id in sample_subjects:
        ...
        # Consistent with user confirmation: sub-xxxx_condition_labels.json
        # Also check metadata variant
                ...
                with open(json_path, 'r') as f:
                    ...
                    if isinstance(data, dict):
                        if "event_labels" in data:
                            # Conditions are strings inside a list
                            conditions.update(set(data["event_labels"]))
                        else:
                            # Fallback: Conditions are keys of the dictionary
...
def run_evaluation(args):
    ...
    if not args.conditions:
        target_conditions = discover_conditions(model_dir, clean_meta_df, args.stage, args.all_layers)
        if not target_conditions:
            logger.error("No conditions found. Check if metadata JSON files exist.")
            return
    ...
    for condition in target_conditions:
        print(f"\n" + "="*60)
        print(f" CBraMod Evaluation: {condition} ".center(60, "="))
        layer_info = f"Layer {args.layer_idx}" if args.layer_idx is not None else args.all_layers
        print(f" All Layers: {layer_info} | Representation: {args.representation} ".center(60, "-"))
        ...
        try:
            container = load_temp_dl_data(
                ...
                model_size=None, # CBraMod doesn't use REVE-style sizes
                pooling=None,    # CBraMod doesn't use REVE-style pooling
        ...
        row = {
            ...
            "size": "NA",
            "pooling": "NA",
            "last_layer": "yes" if not args.all_layers else f"layer_{args.layer_idx}" if args.layer_idx is not None else "all_layers",
        ...
        # Optional JSON output for per-layer tracking
        if args.json_dir:
            json_dir = Path(args.json_dir)
            json_dir.mkdir(parents=True, exist_ok=True)
            layer_tag = f"layer_{args.layer_idx}" if args.layer_idx is not None else "base"
            json_path = json_dir / f"eval_{layer_tag}_{condition}.json"
            with open(json_path, "w") as f:
                json.dump(row, f, indent=4)

    print(f"\nCBraMod evaluations complete. Results saved to: {args.csv_out}")
