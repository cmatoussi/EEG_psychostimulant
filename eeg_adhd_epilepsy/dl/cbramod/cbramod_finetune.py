...
# Default Hyperparameters (Old Implementation style: Single Pulse Fine-tuning)
BATCH_SIZE = 64
EPOCHS = 30
LEARNING_RATE = 1e-4
WEIGHT_DECAY = 5e-2
...
def get_data_indices(data_root, label_csv, target_conditions=None):
    ...
    epoch_metadata, all_labels, all_subjects, ch_names = [], [], [], None
...
def train_fold(fold_idx, train_loader, val_loader, num_channels, num_segments, patch_size, weights_path, 
               lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY, dropout=0.1, patience=10, 
               class_weights=None):
    """
    Standard Single-Phase Global Fine-tuning (Old Implementation)
    """
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=math.ceil(patience/2))
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"      Epoch {epoch+1}/{EPOCHS} | Loss: {train_loss/len(train_loader):.4f} | Val AUC: {val_auc:.4f} | Val BAcc: {val_bacc:.4f}")
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(epoch_metadata, y, groups=subjects)):
        ...
        if args.tune:
            def objective(trial):
                t_lr = trial.suggest_float("lr", 1e-6, 1e-3, log=True)
                t_drop = trial.suggest_float("dropout", 0.05, 0.5)
                _, t_metrics = train_fold(fold, train_loader, val_loader, 
                                          num_channels, NUM_SEGMENTS, PATCH_SIZE, 
                                          args.weights_path, lr=t_lr, dropout=t_drop, 
                                          patience=args.patience, class_weights=class_weights)
                return t_metrics['auc']
            
            study = optuna.create_study(direction="maximize")
            study.optimize(objective, n_trials=args.n_trials)
            lr, dropout = study.best_params['lr'], study.best_params['dropout']
            best_params = study.best_params
        else:
            lr, dropout = LEARNING_RATE, 0.1
            best_params = {"lr": lr, "dropout": dropout}

        model, metrics = train_fold(fold, train_loader, val_loader, 
                                     num_channels, NUM_SEGMENTS, PATCH_SIZE, 
                                     args.weights_path, lr=lr, dropout=dropout, 
                                     patience=args.patience, class_weights=class_weights)
        fold_results.append(metrics)
        
        save_data = {"metrics": metrics, "best_params": best_params, "condition": cond_suffix, "fold": fold + 1}
    ...
    if args.fold is None or len(fold_results) == 5:
        ...
        for i, m in enumerate(fold_results):
            ...
            rows.append({'Fold': i+1, 'Condition': cond_suffix, 'Val_AUC': metrics['auc'], 'Val_Acc': metrics['acc'], 'Val_BAcc': metrics['bacc'], 'Val_F1': metrics['f1']})
