#!/bin/bash
#SBATCH --job-name=reve_og_large
#SBATCH --output=logs/reve_og_large_%j.out
#SBATCH --error=logs/reve_og_large_%j.err
...
echo "Starting REVE-large original-weights evaluation (no fine-tuning)"
echo "Device: $(python -c 'import torch; print(torch.cuda.get_device_name(0) if torch.cuda.is_available() else "CPU")')"

# 3. Run evaluation across all 8 conditions with REVE-large
python /home/mat/projects/EEG_psychostimulant/eeg_adhd_epilepsy/dl/reve/predict_og.py \
    ...
    --out-csv    /home/mat/projects/EEG_psychostimulant/data/results/eval/reve_large_without_finetuning.csv \
    --model-size large \
    --batch-size 8
