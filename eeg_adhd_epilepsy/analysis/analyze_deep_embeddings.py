...
def main():
    ...
    parser.add_argument("--embeddings_dir", type=str, required=True, help="Path to extracted .npy files")
    ...
    parser.add_argument("--desc", type=str, default="base", help="EEG stage descriptor (e.g., base, correctIca)")
    parser.add_argument("--dataset_name", type=str, default="deep_embeddings", help="Name for the output report folder")
    parser.add_argument("--target_col", type=str, default="Diagnosis_Combined", help="Clinical column to predict")
    parser.add_argument("--representation", type=str, default="subject_flat", choices=["epoch_flat", "subject_flat"], help="How to aggregate segments")
    ...
    parser.add_argument("--conditions", nargs="+", default=None, help="Specific conditions to keep (e.g. EO_baseline)")
    ...
    parser.add_argument("--max_samples", type=int, default=50000, help="Max number of segments to process (subsample if exceeded)")
    ...
    out_dir = Path("/home/mat/projects/EEG_psychostimulant/data/results/dim_reduction_reports")
    ...
    for condition in loaded_conditions:
        ...
        if args.target_col == "EO_EC":
            ...
            cond_mask = np.isin(np.asarray(container.coords["condition"]), ["EO_baseline", "EC_baseline"])
            ...
            conditions = np.asarray(temp_container.coords["condition"]).astype(str)
            
            subject_cond_map = {}
            for s, c in zip(study_ids, conditions):
                if s not in subject_cond_map:
                    subject_cond_map[s] = set()
                subject_cond_map[s].add(c)
            
            subjects_with_both = {s for s, cs in subject_cond_map.items() if "EO_baseline" in cs and "EC_baseline" in cs}
            
            # 3. Final mask: rows in ["EO_baseline", "EC_baseline"] AND subject in subjects_with_both
            final_mask = np.array([
                (c in ["EO_baseline", "EC_baseline"]) and (s in subjects_with_both)
        ...
        if args.target_col == "EO_EC":
            # After aggregation, string coords (like 'condition') may be lost.
            # Reconstruct it by parsing the composite Study ID_condition key.
            composite_keys = np.asarray(
                rep_container.coords.get("Study ID_condition", rep_container.ids)
            ).astype(str)
            # Format: "{study_id}_{condition}" e.g. "0001_EO_baseline"
            ...
            raw_cond_labels = reconstructed_conditions
            raw_labels = (raw_cond_labels == "EO_baseline").astype(int)
        ...
        # Subsampling for performance and memory
        if len(valid_mask) > args.max_samples:
            logger.info(f"Subsampling {len(valid_mask)} segments down to {args.max_samples} for {condition}")
            random.seed(42)
            valid_mask = random.sample(valid_mask, args.max_samples)
            valid_mask.sort() # Keep temporal order if possible
            
        rep_container = rep_container.isel(obs=valid_mask)
        X = rep_container.X
        
        if args.target_col == "EO_EC":
            # condition coord was explicitly reconstructed above and survives isel
            cond_labels = np.asarray(rep_container.coords["condition"]).astype(str)
            labels = (cond_labels == "EO_baseline").astype(int)
    ...
    try:
        ...
        target_suffix = "_EO_EC" if args.target_col == "EO_EC" else ""
        if args.model == "reve":
            # e.g., reve_base_with_pooling_all_layers_subject_flat_EO_EC_report.html
            layer_suffix = "_all_layers" if args.all_layers else ""
            report_filename = f"reve_{args.reve_size}_{args.pooling}_pooling{layer_suffix}_{args.representation}{target_suffix}_report.html"
        elif args.model == "cbramod":
            layer_suffix = "_all_layers" if args.all_layers else "_last_layer"
            report_filename = f"cbramod{layer_suffix}_{args.representation}{target_suffix}_report.html"
